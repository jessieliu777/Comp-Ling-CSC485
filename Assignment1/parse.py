#!/usr/bin/env python3
# Student name: Heng Liu
# Student number: 1004076493
# teach.cs login username: liuheng7
...
class PartialParse(object):
    ...
    def complete(self):
        ...
        """
        # ****BEGIN YOUR CODE****
        if (self.stack == [0] and self.next == len(self.sentence)):
            return True
        else:
            return False
        # ****END YOUR CODE****
    ...
    def parse_step(self, transition_id, deprel=None):
        ...
        """
        # ****BEGIN YOUR CODE****
        # if transition id is left arc
        if transition_id == self.left_arc_id:
            # if there are enough elements in the stack, do left arc
            if (len(self.stack) >= 3):
                result = self.stack.pop(-2)
                self.arcs.append((self.stack[-1], result, deprel))
            else:
                raise ValueError
        # if transition id is right arc
        elif transition_id == self.right_arc_id:
            # if there are enough elements in the stack, do right arc
            if (len(self.stack) >= 2):
                result = self.stack.pop()
                self.arcs.append((self.stack[-1], result, deprel))
            else:
                raise ValueError
        # if transition id is shift
        elif transition_id == self.shift_id:
            # if there are any elements in the buffer, do shift
    ...
    def get_n_leftmost_deps(self, sentence_idx, n=None):
        ...
        for arc in self.arcs:
            (head, dep, _) = arc
            if (head == sentence_idx):
                # if we want all the arcs, add this dep to deps
                if (n == None):
                    deps.append(dep)
                # if we want the first n arcs, if this arc appears
                # at or before nth arc, then add this dep to deps
                elif (n>0):
                    n-=1
                    deps.append(dep)
        # ****END YOUR CODE****
    ...
    def get_n_rightmost_deps(self, sentence_idx, n=None):
        ...
        for i in range (length):
            (head, dep, _) = self.arcs[length - i - 1]
            if (head == sentence_idx):
                # if we want all the arcs, add this dep to deps
                if (n == None):
                    deps.append(dep)
                # if we want the last n arcs, if this arc appears
                # at or after nth arc, then add this dep to deps
                elif (n>0):
                    n-=1
                    deps.append(dep)
        # ****END YOUR CODE****
    ...
    def get_oracle(self, graph: DependencyGraph):
        ...
        *ALSO IMPORTANT* make sure to use the values `self.left_arc_id`,
        ...
        if (self.stack != [0]):
            ...
            right = self.stack[-1]
            left = None
            # if there are at least 2 elements in the stack
            if len(self.stack) >= 3:
                left = self.stack[-2]
                # check if left arc is valid
                if get_head(left, graph) == right:
                    # if so, check for any dependencies for 2nd last element
                    # if there aren't then do left arc
                    transition = self.left_arc_id
                    deprel_label = get_deprel(left, graph)
                    deps = get_deps(left, graph)
                    for dep in deps:
                        # if anything would depend on the second last element
                        if (dep in self.stack or self.next <= dep):
                            # then shift instead of left arc
                            transition = self.shift_id
                            deprel_label = None
                # check if right arc is valid
                elif get_head(right, graph) == left:
                    # if so, check for any dependencies for last element
                    # if there aren't then do right arc
                    transition = self.right_arc_id
                    deprel_label = get_deprel(right, graph)
                    deps = get_deps(right, graph)
                    for dep in deps:
                        # if anything would depend on the last element
                        if (dep in self.stack or self.next <= dep):
                            # then shift instead of right arc
                            transition = self.shift_id
                            deprel_label = None
                # if left arc & right arc aren't valid, then shift
                else:
                    transition = self.shift_id
            # if there are only 2 elements in the stack (root + a word)
            else:
                # if we've gone through all the words in the sentence
                if self.next == len(self.sentence):
                    # do right arc since the only word left is the last word
                    transition = self.right_arc_id
                    deprel_label = get_deprel(right, graph)
                # otherwise, check if we need to do right arc
                else:
                    # if there are no more dependencies then we have to do right arc
                    deps = get_deps(right, graph)
                    transition = self.right_arc_id
                    deprel_label = get_deprel(right, graph)
                    for dep in deps:
                        # if anything would depend on the element
                        if (dep in self.stack or self.next <= dep):
                            # then shift instead of right arc
                            transition = self.shift_id
                            deprel_label = None
                    # if root is not the head, then shift
                    head = get_head(right, graph)
                    if head != 0:
                        transition = self.shift_id
                        deprel_label = None
        # if it's the first step, then we shift
        else:
            transition = self.shift_id
        # ****END YOUR CODE****
...
def minibatch_parse(sentences, model, batch_size):
    ...
    """
    # ****BEGIN YOUR CODE****
    # initialize arcs to be empty list
    arcs = []
    # initialize partial parses to be empty list
    partial_parses = []
    # add all the partial parses of the sentence to the partial parses list
    for s in sentences:
        partial_parses.append(PartialParse(s))
        # make a shallow copy of partial parses 
    unfinished_parses = partial_parses.copy()
    while (unfinished_parses != []):
        minibatch = unfinished_parses[:batch_size]
        td_pairs = model.predict(minibatch)
        i = 0
        while i < len(td_pairs):
            try:
                (transition, deprel) = td_pairs[i]
                minibatch[i].parse_step(transition, deprel)
            except ValueError:
                # if complete. remove from unfinished parses
