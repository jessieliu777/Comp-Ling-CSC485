...
class ParserModel(nn.Module):
    ...
    def create_net_layers(self) -> None:
        ...
        self.outer_layer = nn.Linear(self.config.hidden_size, self.config.n_classes)
        # ****END YOUR CODE****
    ...
    def get_concat_embeddings(self, word_id_batch: torch.Tensor,
        ...
        """
        # ****BEGIN YOUR CODE****
        embed_word = self.word_embed(word_id_batch)
        embed_tag = self.tag_embed(tag_id_batch)
        embed_deprel = self.deprel_embed(deprel_id_batch)
        reshaped_word = self.reshape_embedded(embed_word)
        reshaped_tag = self.reshape_embedded(embed_tag)
        reshaped_deprel = self.reshape_embedded(embed_deprel)
        x = torch.cat((reshaped_word, reshaped_tag, reshaped_deprel), 1)
        # ****END YOUR CODE****
        return x
